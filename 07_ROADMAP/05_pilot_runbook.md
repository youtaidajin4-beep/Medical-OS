...
1. **ブラウザ配置**（5分）
   ...
   - ログイン後は **診療パネル**（`/panel`）が開く

2. **ログイン確認**（2分）
   - アカウントでログイン
   - 診療パネル（「診療を開始」）が表示されること
...
> **補足**: 患者マスタは CLINICS 側です。Medical OS に「患者 / 匿名症例」の区別はありません。履歴・語彙設定はパネル右上の拡大からフル画面で開きます。
...
| # | 項目 | 結果 |
|---|------|------|
| 1 | CLINICS 全画面 + Medical OS 右下（約420×800）で `/panel` 表示 | |
| 2 | 表示名任意で診療開始 → 同意 → 録音30秒以上 → 停止 | |
| 3 | SOAP 生成 → 確認済み → コピー → CLINICS 貼付が **3分以内** | |
| 4 | Case 1 誤認識4語（聴診では / 気管支炎 / ムコダイン / 再診）合格 | |
| 5 | パイプライン失敗時にパネル内で復帰（症例選択へ飛ばない） | |
| 6 | 本番: 固有パスワード・実OpenAI・`/health/ai` で ffmpegAvailable | |

### Compact 主アクション（迷わない導線）

1. 未確認: 下部 sticky **「確認済みにする」** のみ
2. 確認後: **「SOAP をコピー → CLINICS」** + **「書類を全部作る」**
3. パネル内タブ: **SOAP / 相談 / 書類 / 紙資料**（画面遷移なし）
4. 書類は折りたたみを開かなくても sticky から一発生成できること
