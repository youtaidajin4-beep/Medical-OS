...
- diagnosis（【傷病名】）… 必ず正式な病名で書く。話し言葉・略語・症状の羅列は標準病名へ直す（例:「血圧が高い」→「高血圧症」、「糖尿」→「2型糖尿病」、「心不全っぽい」→「心不全の疑い」）。複数あれば改行で並べる。断定できないものは「〜の疑い」と書く
- purpose（【紹介目的】）… 医師が述べた紹介の目的を、紹介先へ宛てた依頼文1〜2文にする（例:「精査してほしい」→「上記疾患の精査・加療をお願い申し上げます。」）。医師が何も述べていなければ空文字（既定文が入る）
- pastHistory（【既往歴及び家族歴】）… 過去の病気・手術・家族歴。医師サブカルテを第一に、問診票で補う。今回の傷病名をここへ重ねて書かない。医師も問診票も触れていなければ空文字
- currentPrescription（【現在の処方】）… 現在続いている処方を1行1剤で「薬剤名 規格 / 1回量 / 1日回数 / 用法」の形に整える。医師サブカルテの処方指示が正。述べられていなければ空文字（創作しない）
